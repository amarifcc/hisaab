...
export async function POST(req: Request) {
  ...
  const { part_id, from_person, amount, date, notes } = body

  if (!part_id || !amount) return NextResponse.json({ error: 'Missing fields' }, { status: 400 })
...
export async function PUT(req: Request) {
  ...
  const body = await req.json()
  const { id, part_id, from_person, amount, date, notes } = body
